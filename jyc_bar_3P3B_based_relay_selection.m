function  [t,location_relay,relay,PDR,location_vehi,num_vehi]=jyc_bar_3P3B_based_relay_selection(location_vehi,locationmark,R,num_vehi,N_iter,N_part,A,Linmap1m,T,ima2,t_relay)
 ...
 while ((isend == 1 && (bar_bef_end==1 || (sum((relay-relayend).^2))^0.5/Linmap1m>R)) || (isend == 0 && (bar_bef_end==1 || (sum((relay-relayend).^2))^0.5/Linmap1m>R/2))) && t<T 
     ...
     if re_symbol==0
         [t,location_relay,n_relay,relay_main,relay,num_vehi,location_vehi,n_re_relay,PDR_part,last_t]...
             =jyc_bar_3P3B_curve_relay_selection(location_vehi_in_Rp,location_vehi,n_relay,R,Linmap1m,num_vehi,location_relay,N_iter,N_part,A,relay,t,posi_opt,n_re_relay,locationmark,ima2,send_end,old_sender,old_posi_opt);
     else
         [location_relay,re_t,num_vehi,location_vehi,location_re_relay,n_re_relay,has_cov_symbol,PDR_part,last_t]...
             =jyc_bar_3P3B_reverse_relay_selection(location_vehi,n_relay,locationmark,R,Linmap1m,num_vehi,location_relay,N_iter,N_part,A,send_start,t,re_t,location_re_relay,n_re_relay,vacant0,vacant,re_symbol,relay_main,has_cov_symbol,T,ima2);
 ...
 if isend==0%如果send_end是普通Junction点
     ...
     if nearest(1,1)~=relay(1,1) || nearest(1,2)~=relay(1,2)%只要出现nearest中存储的X、Y坐标中的某一个与relay的X、Y坐标不对等，那么说明更新过了nearest参数         
         ...
         if dis3 < R %如果在最后一跳relay的上一跳(或在send_start)处就可以直接跳到nearest点，那么就直接把最后一跳relay更新成nearest点             
             ...
             [delay_one_hop_average,PDR_part,relay] = jyc_3P3B_delay_calcu (location_vehi_in_Rp,density_EM,N_iter,N_part,CW_CTB,sender,posi_opt,relay);
