function [posi_opt] = jyc_bar_leftchoose(road_in_relay_R,locationmark,relay,K,K_min,K_max,R,Linmap1m,send_end)
    %由于我们默认反向广播消息方向都是从路段编号小的地方广播至路段编号大的地方，与正向广播相反
    %则我们反向广播时只需要选择 road_in_relay_R 数组中存储的路段编号最大的路段作为我们的Popt所在路段即可
    if length(send_end) ==3%如果是在反向广播，则选择最靠近send_end点的路段点作为反向广播Popt所在路段点,PS：一般左向选择基本都是反向广播
        ...
        Popt_road_index = max(road_in_relay_R); %（反向广播）取出存储的最大的路段编号，因为最大的编号路段与扇形区域边界有交点，可能是半径边界与该路段有交点，可能是弧边界与该路段有交点
    end
    syms x y;
    ...
    eq3 = (y-locationmark(Popt_road_index-1,2))*(locationmark(Popt_road_index,1)-locationmark(Popt_road_index-1,1))-(locationmark(Popt_road_index,2)-locationmark(Popt_road_index-1,2))*(x-locationmark(Popt_road_index-1,1));
    eq3=subs(eq3);
    ...
    if (locationmark(Popt_road_index-1,1)<=x && x<=locationmark(Popt_road_index,1)) || (locationmark(Popt_road_index-1,1)>=x && x>=locationmark(Popt_road_index,1))%如果求出的x在Popt_road_index与Popt_road_index-1构成的路段上
        ...
        if (locationmark(Popt_road_index,1)<=x && x<=locationmark(Popt_road_index+1,1)) || (locationmark(Popt_road_index,1)>=x && x>=locationmark(Popt_road_index+1,1))%如果这回求出的x在Popt_road_index与Popt_road_index+1构成的路段上
            ...
            eq4=subs(eq4);
            %先求圆弧边界与前向路段的交点，看是否满足要求
            eq5 = (y-locationmark(Popt_road_index-1,2))*(locationmark(Popt_road_index,1)-locationmark(Popt_road_index-1,1))-(locationmark(Popt_road_index,2)-locationmark(Popt_road_index-1,2))*(x-locationmark(Popt_road_index-1,1));%以路段标记点编号为Popt_road_index和Popt_road_index-1的两点，做出直线eq5
            eq5=subs(eq5);
            ...
            if isempty(X) || isempty(Y)%如果发现-1和0路段与圆弧的交点不是我们要求的点，则求0和+1路段与圆弧边界的交点，即求后向路段与弧边界的交点
                syms x y;
                ...
                eq6=subs(eq6);
                [x,y]=solve(eq4,eq6);%求圆eq4与路段直线eq6的交点
                x = eval([x]);
                y = eval([y]);
