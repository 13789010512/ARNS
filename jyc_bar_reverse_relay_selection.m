function [location_relay,re_t,num_vehi,location_vehi,location_re_relay,n_re_relay,has_cov_symbol,PDR,last_t]=jyc_bar_reverse_relay_selection(location_vehi,n_relay,locationmark,R,Linmap1m,num_vehi,location_relay,N_iter,N_part,A,send_start,t,re_t,location_re_relay,n_re_relay,vacant0,vacant,re_symbol,relay_main,has_cov_symbol,T,ima2)
    %反向广播情况
...
%             hold on
            ...
            if N_part==4%如果是log算法
                [delay_one_hop_mini_average,PDR_part] = jyc_log_partition (location_vehi_in_Rp,density_EM,N_iter,N_part,CW_CTB,sender,posi_opt,re_relay);
                re_t = re_t+delay_one_hop_mini_average;
                PDR = [PDR;PDR_part];%while循环中无限加PDR_part至PDR数组中
            else        %否则就是N_part==3，即3P3B算法
                [delay_one_hop_average,PDR_part] = jyc_3P3B_delay_calcu (location_vehi_in_Rp,density_EM,N_iter,N_part,CW_CTB,sender,posi_opt,re_relay);
...
%             hold on
    ...
    if N_part==4%如果是log算法
        last_t = delay_one_hop_mini_average;
    else        %否则就是N_part==3，即3P3B算法
