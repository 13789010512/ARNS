 function  [relay_road_index]=jyc_find_relayroad_near_junction(locationmark,relay,R,Linmap1m)
 %本函数是jyc_find_relayroad的特例函数，专用于relay覆盖范围超出了junction点（send_end点）的情况，在此情况中，通信范围内的车辆节点超出了locationmark的可选范围，我们将其所在路段置为-1
    ...
    if isempty(relay_road_index)%用来解决车辆节点超出了本道路在下一道路上的情况，此情况车辆节点所在路段我们置为-1
        if (sum((locationmark(1,1:2)-relay(1:2)).^2))^0.5/Linmap1m < R/2
            relay_road_index = -1;
        else
