...
%我们改为以Popt点为初始0位置，以通信范围内可供通信的节点与Popt的相对距离为inform_vehi(:,4)   9.21日
%    参数初始化
...
%idea：考虑将relay_y当作Popt，定为0号位置，通信范围内其他车辆的位置都以与Popt的距离代替，当然这里指的“其他车辆”不包括与消息传播方向相反的车辆节点（该类车辆节点已经被筛选出去了）
...
relay_y=0;%起点为0
n_seg_min = (2^(N_part-1))^N_iter;% 在传输范围内最小segmennt的个数
...
while(1)
    ...
    %parttion in one hop
    [~,t_partition_mini,bound_left,bound_right,inform_vehi]=jyc_partition(relay_y,relay,R,inform_vehi,N_iter,N_part);%分区延时计算代码（主要修改）
